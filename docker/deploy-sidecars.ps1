...
if ($subscriptionId -eq "help" -or $webAppName -eq "help" -or $resourceGroup -eq "help" -or $registryUrl -eq "help" -or $base64DockerCompose -eq "help" -or $mainServiceName -eq "help" -or $targetPort -eq "help") {
    ...
    Write-Output "`n  dockerComposePath            : Path to Docker Compose YAML"
    ...
    Write-Output "`n./update-webapp.ps1 -subscriptionId <subscriptionId> -webAppName <webAppName> -resourceGroup <resourceGroup> -registryUrl <registryUrl> -dockerComposePath <dockerComposePath> -mainServiceName <mainServiceName> -targetPort <targetPort>"
    Write-Output "`n"
    Write-Output "`nSample Command:"
    Write-Output "`n./update-webapp.ps1 -subscriptionId 'your_subscription_id' -webAppName 'your_web_app_name' -resourceGroup 'your_resource_group' -registryUrl 'your_registry_url' -dockerComposePath 'path_to_your_docker-compose_yaml' -mainServiceName 'your_main_service_name' -targetPort 'your_target_port'"
    exit
}
...
$dockerComposeYaml = docker-compose -f $dockerComposePath config 
Write-Output "`Docker Compose YAML:`n$dockerComposeYaml"
