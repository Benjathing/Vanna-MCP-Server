...
$tempYamlPath = "" # Initialize to empty string
...
try {
    ...
    Write-Host "Checking for existing container app '$ContainerAppName'..."
    $appExists = az containerapp show --name $ContainerAppName --resource-group $ResourceGroupName -o tsv

    if (-not $appExists) {
        Write-Host "Container app not found. Starting creation process..."
        ...
        Write-Host -f Cyan "Container app found. Skipping creation and proceeding to update."
    }
